...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "List of PR To Be Cancel", path: "/purchasing/purchase-requisition/to-be-cancel-list" },
  ],
});

const { $swal } = useNuxtApp();

const toBeCancelList = ref([]);
...
const filteredToBeCancelList = ref([...toBeCancelList.value]);

const applyFilters = () => {
  let filtered = [...toBeCancelList.value];
  ...
  filteredToBeCancelList.value = [];
  nextTick(() => {
    filteredToBeCancelList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredToBeCancelList.value.length);
...
const fetchToBeCancel = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-requisition/list-pr-to-be-cancel", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      toBeCancelList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch PR to be cancel list",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching PR to be cancel list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of PR To Be Cancel</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="to-be-cancel-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`to-be-cancel-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredToBeCancelList"
...
<style scoped>
.to-be-cancel-table-wrapper :deep(.table-header) {
  display: none;
}

.to-be-cancel-table-wrapper :deep(.rs-table thead th) {
