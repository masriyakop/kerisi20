...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "List of Purchase Requisition Cancellation", path: "/purchasing/purchase-requisition/cancellation-list" },
  ],
});
...
const fetchCancellations = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-requisition/list-cancellation", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of Purchase Requisition Cancellation</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="cancellation-list-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`cancellation-list-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredCancellationList"
              :field="['no', 'Requisition No', 'Title', 'Total Amount', 'Organization Unit', 'PTJ', 'Fund Type', 'Cost Centre', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.cancellation-list-table-wrapper :deep(.table-header) {
  display: none;
}

.cancellation-list-table-wrapper :deep(.rs-table thead th) {
