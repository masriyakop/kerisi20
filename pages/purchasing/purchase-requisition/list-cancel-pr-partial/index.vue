...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "List of Cancel PR Partial", path: "/purchasing/purchase-requisition/cancel-partial-list" },
  ],
});
...
const applyFilters = () => {
  ...
  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    ...
    filtered = filtered.filter((item) => {
      ...
      const amount = (item["Total Amount"] || "").toString().toLowerCase();
...
const fetchCancelPartial = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-requisition/list-cancel-pr-partial", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      cancelPartialList.value = (data.value.data || []).map((item, index) => ({
        ...
        "Amount Partial PR (RM)": item.partial_amount || 0,
        "Organization Unit": '',
        PTJ: item.oun_code || '',
        Action: "",
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch cancel partial list",
        icon: "error",
      });
    }
  } catch (error) {
    console.error("Error fetching cancel partial list:", error);
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching cancel partial list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of Cancel Partial PR</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="cancel-partial-pr-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`cancel-partial-pr-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredCancelPartialList"
              :field="['no', 'Id', 'Requisition No', 'Title', 'Total Amount', 'Amount Partial PR (RM)', 'Organization Unit', 'PTJ', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:AmountPartialPRRM="data">
                <div class="text-right">
                  {{ toCurrency(data.value['Amount Partial PR (RM)']) }}
                </div>
              </template>
              <template v-slot:OrganizationUnit="data">
                {{ data.value['Organization Unit'] }}
              </template>
...
<style scoped>
.cancel-partial-pr-table-wrapper :deep(.table-header) {
  display: none;
}

.cancel-partial-pr-table-wrapper :deep(.rs-table thead th) {
