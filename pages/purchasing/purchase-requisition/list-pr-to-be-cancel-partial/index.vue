...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "List of PR To Be Cancel Partial", path: "/purchasing/purchase-requisition/to-be-cancel-partial-list" },
  ],
});

const { $swal } = useNuxtApp();

const toBeCancelPartialList = ref([]);
...
const filteredToBeCancelPartialList = ref([...toBeCancelPartialList.value]);

const applyFilters = () => {
  let filtered = [...toBeCancelPartialList.value];

  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    const keyword = searchKeyword.value.toLowerCase().trim();
    filtered = filtered.filter((item) => {
      const requisitionNo = (item["Requisition No"] || "").toString().toLowerCase();
      const requestBy = (item["Request By"] || "").toString().toLowerCase();
      const fundType = (item["Fund Type"] || "").toString().toLowerCase();
      const costCentre = (item["Cost Centre"] || "").toString().toLowerCase();
      const activityCode = (item["Activity Code"] || "").toString().toLowerCase();
      const title = (item.Title || "").toString().toLowerCase();

      return (
        requisitionNo.includes(keyword) ||
        requestBy.includes(keyword) ||
        fundType.includes(keyword) ||
        costCentre.includes(keyword) ||
        activityCode.includes(keyword) ||
        title.includes(keyword)
      );
    });
  }

  filteredToBeCancelPartialList.value = [];
  nextTick(() => {
    filteredToBeCancelPartialList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredToBeCancelPartialList.value.length);
...
const fetchToBeCancelPartial = async () => {
  try {
    loading.value = true;
    const queryParams = {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-requisition/list-pr-to-be-cancel-partial", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      toBeCancelPartialList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        Id: item.rqm_requisition_id || '',
        "Requisition No": item.rqm_requisition_no || '',
        "Request By": item.rqm_request_by || '',
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch PR to be cancel partial list",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching PR to be cancel partial list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: `Do you want to delete "${item["Requisition No"]}"?`,
    icon: "warning",
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of PR To Be Cancel Partial</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="to-be-cancel-partial-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`to-be-cancel-partial-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredToBeCancelPartialList"
              :field="['no', 'Id', 'Requisition No', 'Request By', 'Fund Type', 'Cost Centre', 'Activity Code', 'Title', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:RequestBy="data">
                {{ data.value['Request By'] }}
              </template>
...
<style scoped>
.to-be-cancel-partial-table-wrapper :deep(.table-header) {
  display: none;
}

.to-be-cancel-partial-table-wrapper :deep(.rs-table thead th) {
