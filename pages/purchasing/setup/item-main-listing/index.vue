...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Item Main Listing", path: "/purchasing/setup/item-main-listing" },
  ],
});
...
const filteredItemList = ref([...itemList.value]);
...
const applyFilters = () => {
  ...
  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    ...
    filtered = filtered.filter((item) => {
      ...
      const itemDesc = (item["Item Description"] || "").toString().toLowerCase();
      const accountCode = (item["Account Code"] || "").toString().toLowerCase();
      const categoryCode = (item["Category Code"] || "").toString().toLowerCase();
  ...
  nextTick(() => {
    filteredItemList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredItemList.value.length);
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/setup/item-main-listing", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      itemList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        ...
        "Subcategory Code": item.SubcategoryCode || '',
        "Subsiri Code": item.SubsiriCode || '',
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch item main listing",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching item main listing",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: `Do you want to delete "${item["Item Code"]}"?`,
    icon: "warning",
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Item Main Listing</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="item-main-listing-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`item-main-listing-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredItemList"
              :field="['no', 'Item Code', 'Item Description', 'Account Code', 'Category Code', 'Description', 'Subcategory Code', 'Subsiri Code', 'Category', 'Level', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:CategoryCode="data">
                {{ data.value['Category Code'] }}
              </template>
              <template v-slot:Description="data">
                {{ data.value.Description }}
              </template>
              <template v-slot:SubcategoryCode="data">
                {{ data.value['Subcategory Code'] }}
              </template>
              <template v-slot:SubsiriCode="data">
                {{ data.value['Subsiri Code'] }}
              </template>
...
<style scoped>
.item-main-listing-table-wrapper :deep(.table-header) {
  display: none;
}

.item-main-listing-table-wrapper :deep(.rs-table thead th) {
