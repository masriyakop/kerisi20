<script setup>
definePageMeta({
  title: "List Of Assessment Question",
  middleware: ["auth"],
  requiresAuth: true,
  breadcrumb: [
    ...
    { name: "Assessment Question", path: "/purchasing/setup/assessment-question" },
  ],
});

const { $swal } = useNuxtApp();

const pageName = "List Of Assessment Question";
const moduleName = "Purchasing";
const pageBreadcrumbText = "Dashboard > Purchasing > Setup > Assessment Question";
...
const assessmentQuestionList = ref([]);
...
const filteredAssessmentQuestionList = ref([...assessmentQuestionList.value]);

const applyFilters = () => {
  let filtered = [...assessmentQuestionList.value];
  ...
  filteredAssessmentQuestionList.value = [];
  nextTick(() => {
    filteredAssessmentQuestionList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredAssessmentQuestionList.value.length);
...
const fetchAssessmentQuestions = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/setup/assessment-question", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      assessmentQuestionList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        Id: item.id || '',
        Code: item.code || '',
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch assessment questions",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching assessment questions",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const messageText = `Are you sure? Do you want to delete "${item.Code}"?`;
...
const {
  ...
  pageName: "Assessment Question",
  apiDataPath: "/api/purchasing/setup/assessment-question",
  defaultExportColumns: ["Code", "Description", "Status"],
  getFilteredList: () => filteredAssessmentQuestionList.value,
  datatableRef,
  searchKeyword,
  smartFilter,
  applyFilters,
  smartFilterLabels: { Status: "Status" },
    ...
    <rs-card>
      <template #header>
        <div class="flex justify-between items-center">
          <div class="text-lg font-semibold">List Of Assessment Question</div>
          <rs-dropdown
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <div class="assessment-question-table-wrapper">
            ...
            <rs-table
              ...
              :key="`assessment-question-table`"
              :data="filteredAssessmentQuestionList"
              :field="['no', 'Id', 'Code', 'Description', 'Status', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.assessment-question-table-wrapper :deep(.table-header) {
  display: none;
}

.assessment-question-table-wrapper :deep(.rs-table thead th) {
