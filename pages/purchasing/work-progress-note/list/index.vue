...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Work Progress Note List", path: "/purchasing/work-progress-note/list" },
  ],
});
...
const fetchWPN = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/work-progress-note/list", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      wpnList.value = (data.value.data || []).map((item, index) => ({
        ...
        "Tax Amount<br>(RM)": item.tax_amount || 0,
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch work progress notes",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching work progress notes",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of Work Progress Note</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="wpn-list-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`wpn-list-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredWPNList"
              :field="['no', 'WPN No', 'PO No', 'Vendor Code', 'Vendor Name', 'PO Description', 'Tax Amount<br>(RM)', 'Amount<br>(RM)', 'Status', 'Assessment Status', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.wpn-list-table-wrapper :deep(.table-header) {
  display: none;
}

.wpn-list-table-wrapper :deep(.rs-table thead th) {
