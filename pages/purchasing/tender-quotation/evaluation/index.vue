...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Evaluation", path: "/purchasing/tender-quotation/evaluation" },
  ],
});
...
const fetchTenderQuotations = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/tender-quotation/evaluation", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch tender/quotation for evaluation list",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching tender/quotation for evaluation list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of Tender/Quotation for Evaluation</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="evaluation-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`evaluation-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredTenderQuotationList"
              :field="['no', 'Quotation / Tender No', 'Title', 'Reference No', 'Estimated Price (RM)', 'Type', 'PR No', 'Status', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.evaluation-table-wrapper :deep(.table-header) {
  display: none;
}

.evaluation-table-wrapper :deep(.rs-table thead th) {
