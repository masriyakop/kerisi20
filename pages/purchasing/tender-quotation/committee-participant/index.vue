...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Committe & Participant", path: "/purchasing/tender-quotation/committee-participant" },
  ],
});

const { $swal } = useNuxtApp();

const tenderQuotationList = ref([]);
...
const filteredTenderQuotationList = ref([...tenderQuotationList.value]);

const applyFilters = () => {
  let filtered = [...tenderQuotationList.value];

  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    const keyword = searchKeyword.value.toLowerCase().trim();
    filtered = filtered.filter((item) => {
      const tenderQuotationNo = (item["Tender/Quotation No"] || "").toString().toLowerCase();
      const refNo = (item["Reference No"] || "").toString().toLowerCase();
      const title = (item.Title || "").toString().toLowerCase();
      const status = (item.Status || "").toString().toLowerCase();

      return (
        tenderQuotationNo.includes(keyword) ||
  ...
  filteredTenderQuotationList.value = [];
  nextTick(() => {
    filteredTenderQuotationList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredTenderQuotationList.value.length);
...
const fetchTenderQuotations = async () => {
  try {
    loading.value = true;
    const queryParams = {
    ...
    const { data } = await useFetch("/api/purchasing/tender-quotation/committee-participant", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      tenderQuotationList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        "Tender/Quotation No": item.tender_quotation_no || '',
        "Reference No": item.reference_no || '',
        "Start Date": item.start_date || '',
        "End Date": item.end_date || '',
        Title: item.title || '',
        "Estimated Price": item.estimated_price || 0,
        Status: item.status || '',
        Action: "",
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch tender/quotation list",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching tender/quotation list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: `Do you want to delete "${item["Tender/Quotation No"]}"?`,
    icon: "warning",
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List Of Tender/Quotation</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="committee-participant-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`committee-participant-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredTenderQuotationList"
              :field="['no', 'Tender/Quotation No', 'Reference No', 'Start Date', 'End Date', 'Title', 'Estimated Price', 'Status', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:TenderQuotationNo="data">
                {{ data.value['Tender/Quotation No'] }}
              </template>
              ...
              <template v-slot:EstimatedPrice="data">
                <div class="text-right">
                  {{ toCurrency(data.value['Estimated Price']) }}
                </div>
              </template>
...
<style scoped>
.committee-participant-table-wrapper :deep(.table-header) {
  display: none;
}

.committee-participant-table-wrapper :deep(.rs-table thead th) {
