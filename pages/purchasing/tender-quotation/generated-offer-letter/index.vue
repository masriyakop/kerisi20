...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Generated Offer Letter", path: "/purchasing/tender-quotation/generated-offer-letter" },
  ],
});
...
const applyFilters = () => {
  ...
  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    ...
    filtered = filtered.filter((item) => {
      ...
      const vendor = (item.Vendor || "").toString().toLowerCase();
...
const fetchTenderQuotations = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/tender-quotation/generated-offer-letter", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    <rs-card>
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <div class="generated-offer-letter-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`generated-offer-letter-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredTenderQuotationList"
              :field="['no', 'Quotation/Tender No', 'Reference No', 'Start Date', 'End Date', 'Title', 'Vendor', 'Price (RM)', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.generated-offer-letter-table-wrapper :deep(.table-header) {
  display: none;
}

.generated-offer-letter-table-wrapper :deep(.rs-table thead th) {
