...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Selection", path: "/purchasing/tender-quotation/selection" },
  ],
});
...
const fetchTenderQuotations = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/tender-quotation/selection", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch tender/quotation/DP for selection list",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching tender/quotation/DP for selection list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List Of Tender/Quotation/DP for Selection</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="selection-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`selection-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredTenderQuotationList"
              :field="['no', 'Quotation/Tender No', 'Reference No', 'Start Date', 'End Date', 'Title', 'Estimated Price', 'PR No', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.selection-table-wrapper :deep(.table-header) {
  display: none;
}

.selection-table-wrapper :deep(.rs-table thead th) {
