<script setup>
definePageMeta({
  title: "Good Receive Note Cancel List",
  middleware: ["auth"],
  requiresAuth: true,
  breadcrumb: [
    ...
    { name: "Good Receive Note Cancel List", path: "/purchasing/good-receive-note/cancel-list" },
  ],
});

const { $swal } = useNuxtApp();

const grnCancelList = ref([]);
...
const filteredGRNCancelList = ref([...grnCancelList.value]);

const applyFilters = () => {
  let filtered = [...grnCancelList.value];

  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    const keyword = searchKeyword.value.toLowerCase().trim();
    filtered = filtered.filter((item) => {
      const grnNo = (item["Good Receive Note No"] || "").toString().toLowerCase();
  ...
  filteredGRNCancelList.value = [];
  nextTick(() => {
    filteredGRNCancelList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredGRNCancelList.value.length);
...
const fetchGRNCancel = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/good-receive-note/cancel", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      grnCancelList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        "Good Receive Note No": item.grm_receive_no || '',
        ...
        "Good Receive Note date": item.grm_receive_date || '',
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch good receive note cancel list",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching good receive note cancel list",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: `Do you want to delete "${item["Good Receive Note No"]}"?`,
    icon: "warning",
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Good Receive Note Cancel List</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="grn-cancel-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`grn-cancel-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredGRNCancelList"
              :field="['no', 'Good Receive Note No', 'Purchase Order No', 'Vendor Code', 'vendor Name', 'PO Description', 'Good Receive Note date', 'Amount (RM)', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:GoodReceiveNoteNo="data">
                {{ data.value['Good Receive Note No'] }}
              </template>
              ...
              <template v-slot:GoodReceiveNotedate="data">
                {{ data.value['Good Receive Note date'] }}
              </template>
...
<style scoped>
.grn-cancel-table-wrapper :deep(.table-header) {
  display: none;
}

.grn-cancel-table-wrapper :deep(.rs-table thead th) {
