<script setup>
definePageMeta({
  title: "PO Report Listing (Bendahari)",
  middleware: ["auth"],
  requiresAuth: true,
  breadcrumb: [
    { name: "Dashboard", path: "/dashboard" },
    { name: "Purchasing", path: "/purchasing" },
    { name: "Report PO", path: "/purchasing/report-po" },
    { name: "Bendahari", path: "/purchasing/report-po/bendahari" },
  ],
});

const { $swal } = useNuxtApp();

const poReportList = ref([]);
...
const filteredPOReportList = ref([...poReportList.value]);

const applyFilters = () => {
  let filtered = [...poReportList.value];
  ...
  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    ...
    filtered = filtered.filter((item) => {
      ...
      const status = (item["Order Status"] || "").toString().toLowerCase();
  ...
  filteredPOReportList.value = [];
  nextTick(() => {
    filteredPOReportList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredPOReportList.value.length);
...
const fetchPOReport = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/report-po/bendahari", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      poReportList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        "Order ID": item.pom_order_id || '',
        "Purchase Order No": item.pom_order_no || '',
        Description: item.pom_description || '',
        "Amount (RM)": item.pom_order_amt_rm || 0,
        "Vendor Code": item.vcs_vendor_code || '',
        "Vendor Name": item.vcs_vendor_name || '',
        "Order Status": item.pom_order_status || '',
        Action: "",
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch PO report listing",
        icon: "error",
      });
    }
  } catch (error) {
    console.error("Error fetching PO report listing:", error);
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching PO report listing",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">PO Report Listing (Bendahari)</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="po-report-bendahari-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`po-report-bendahari-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredPOReportList"
              :field="['no', 'Order ID', 'Purchase Order No', 'Description', 'Amount (RM)', 'Vendor Code', 'Vendor Name', 'Order Status', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:OrderStatus="data">
...
<style scoped>
.po-report-bendahari-table-wrapper :deep(.table-header) {
  display: none;
}

.po-report-bendahari-table-wrapper :deep(.rs-table thead th) {
