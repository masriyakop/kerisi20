...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "PTJ", path: "/purchasing/report-po/ptj" },
  ],
});
...
const fetchPOReport = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/report-po/ptj", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">PO Report Listing (OU)</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="po-report-ptj-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`po-report-ptj-table-${searchKeyword || 'all'}-${pageSize}`"
...
<style scoped>
.po-report-ptj-table-wrapper :deep(.table-header) {
  display: none;
}

.po-report-ptj-table-wrapper :deep(.rs-table thead th) {
