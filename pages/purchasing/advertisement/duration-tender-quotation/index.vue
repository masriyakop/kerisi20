...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Duration Tender/Quotation", path: "/purchasing/advertisement/duration-tender-quotation" },
  ],
});
...
const applyFilters = () => {
  ...
  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    ...
    filtered = filtered.filter((item) => {
      ...
      const address = (item["Alamat Hantaran Sebutharga"] || "").toString().toLowerCase();
...
const fetchAdvertisements = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/advertisement/duration-tender-quotation", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      advertisementList.value = (data.value.data || []).map((item, index) => ({
        ...
        "Alamat Hantaran Sebutharga": item.alamat_hantaran || '',
        "Estimate Siap": item.estimate_siap || '',
        Type: item.type || '',
    ...
    <rs-card>
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <div class="duration-tender-quotation-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`duration-tender-quotation-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredAdvertisementList"
              :field="['no', 'Quotation/Tender No', 'Reference No', 'Alamat Hantaran Sebutharga', 'Estimate Siap', 'Type', 'Start Date Project', 'End Date Project', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:AlamatHantaranSebutharga="data">
                {{ data.value['Alamat Hantaran Sebutharga'] }}
              </template>
              <template v-slot:EstimateSiap="data">
                {{ data.value['Estimate Siap'] }}
              </template>
...
<style scoped>
.duration-tender-quotation-table-wrapper :deep(.table-header) {
  display: none;
}

.duration-tender-quotation-table-wrapper :deep(.rs-table thead th) {
