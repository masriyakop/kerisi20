...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Senarai Tempoh Masa LPO", path: "/purchasing/report/laporan-perolehan-ptj-jpka-lppm/senarai-tempoh-masa-lpo" },
  ],
});
...
const fetchReport = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/report/laporan-perolehan-ptj-jpka-lppm/senarai-tempoh-masa-lpo", {
      method: "GET",
      query,
      initialCache: false,
    });
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      ...
      <template #body>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium mb-2">OU:</label>
            <FormKit
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Senarai Tempoh Masa LPO</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="senarai-tempoh-masa-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`senarai-tempoh-masa-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredReportList"
              :field="['no', 'No.Rujukan PRE', 'Tarikh Kelulusan PRE', 'Perkara PRE', 'Amaun PRE (RM)', 'Status PRE', 'No.Rujukan POR', 'Tarikh Kelulusan POR', 'Perkara POR', 'Amaun POR (RM)', 'Status POR', 'Nama Pembekal', 'Alamat Pembekal', 'Status Pembekal', 'OU']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.senarai-tempoh-masa-table-wrapper :deep(.table-header) {
  display: none;
}

.senarai-tempoh-masa-table-wrapper :deep(.rs-table thead th) {
