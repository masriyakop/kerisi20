...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "New PO Cancellation", path: "/purchasing/purchase-order/new-cancellation" },
  ],
});
...
const fetchPOs = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-order/new-cancellation", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    <rs-card>
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <div class="new-po-cancellation-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`new-po-cancellation-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredPOList"
              :field="['no', 'pom_order_id', 'Purchase Order No', 'Description', 'Vendor Code', 'Vendor Name', 'Amount', 'Status PO', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
...
<style scoped>
.new-po-cancellation-table-wrapper :deep(.table-header) {
  display: none;
}

.new-po-cancellation-table-wrapper :deep(.rs-table thead th) {
