...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "New Cancel PO Partial", path: "/purchasing/purchase-order/new-cancel-partial" },
  ],
});
...
const fetchPOs = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-order/new-cancel-po-partial", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of Purchase Order</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="new-cancel-po-partial-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`new-cancel-po-partial-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredPOList"
              :field="['no', 'pom_order_id', 'Purchase Order No', 'Description', 'PTJ', 'Vendor Code', 'Vendor Name', 'Amount', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:pom_order_id="data">
                {{ data.value.pom_order_id }}
              </template>
...
<style scoped>
.new-cancel-po-partial-table-wrapper :deep(.table-header) {
  display: none;
}

.new-cancel-po-partial-table-wrapper :deep(.rs-table thead th) {
