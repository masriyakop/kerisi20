...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "List of Purchase Order Cancellation", path: "/purchasing/purchase-order/cancellation-list" },
  ],
});

const { $swal } = useNuxtApp();

const cancellationList = ref([]);
...
const filteredCancellationList = ref([...cancellationList.value]);

const applyFilters = () => {
  let filtered = [...cancellationList.value];
  ...
  filteredCancellationList.value = [];
  nextTick(() => {
    filteredCancellationList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredCancellationList.value.length);
...
const fetchCancellations = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-order/list-cancellation", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      cancellationList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        ...
        "Status PO Cancel": item.pom_order_status || '',
        "Cancel Remark": item.pom_cancel_remark || '',
        Action: "",
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch cancellations",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching cancellations",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List Of Purchase Order Cancellation</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="cancellation-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`cancellation-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredCancellationList"
              :field="['no', 'Purchase Order No', 'Description', 'PTJ/Entry', 'Vendor Code', 'Vendor Name', 'Amount', 'Status PO ', 'Status PO Cancel', 'Cancel Remark', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:StatusPOCancel="data">
                {{ data.value['Status PO Cancel'] }}
              </template>
              <template v-slot:CancelRemark="data">
                {{ data.value['Cancel Remark'] }}
              </template>
...
<style scoped>
.cancellation-table-wrapper :deep(.table-header) {
  display: none;
}

.cancellation-table-wrapper :deep(.rs-table thead th) {
