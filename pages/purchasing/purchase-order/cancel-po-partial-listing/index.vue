...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Cancel PO Partial Listing", path: "/purchasing/purchase-order/cancel-partial-listing" },
  ],
});

const { $swal } = useNuxtApp();

const cancelPartialList = ref([]);
...
const filteredCancelPartialList = ref([...cancelPartialList.value]);

const applyFilters = () => {
  let filtered = [...cancelPartialList.value];

  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    const keyword = searchKeyword.value.toLowerCase().trim();
    filtered = filtered.filter((item) => {
      const poNo = (item["Purchase Order No"] || "").toString().toLowerCase();
      const ptj = (item["PTJ/Entry"] || "").toString().toLowerCase();
      ...
      const status = (item["Status PO"] || "").toString().toLowerCase();
  ...
  if (smartFilter.value.Status) {
    filtered = filtered.filter((item) => item["Status PO"] === smartFilter.value.Status);
  }

  filteredCancelPartialList.value = [];
  nextTick(() => {
    filteredCancelPartialList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredCancelPartialList.value.length);
...
const fetchCancelPartial = async () => {
  try {
    loading.value = true;
    const queryParams = {
    ...
    const { data } = await useFetch("/api/purchasing/purchase-order/cancel-po-partial-listing", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      cancelPartialList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        "Purchase Order No": item.pom_order_no || '',
        "PTJ/Entry": item.oun_code || '',
        ...
        "Amount Partial PO (RM)": item.partial_amount || 0,
        "Status PO": item.pom_order_status || '',
        "Status Cancel PO Partial": item.pom_order_status || '',
        Action: "",
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch cancel partial listing",
        icon: "error",
      });
    }
  } catch (error) {
    console.error("Error fetching cancel partial listing:", error);
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching cancel partial listing",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: `Do you want to delete "${item["Purchase Order No"]}"?`,
    icon: "warning",
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List Of Cancel PO Partial</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="cancel-partial-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`cancel-partial-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredCancelPartialList"
              :field="['no', 'Purchase Order No', 'PTJ/Entry', 'Description', 'Vendor Code', 'Vendor Name', 'Amount', 'Amount Partial PO (RM)', 'Status PO', 'Status Cancel PO Partial', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:PTJEntry="data">
                {{ data.value['PTJ/Entry'] }}
              </template>
              ...
              <template v-slot:AmountPartialPORM="data">
                <div class="text-right">
                  {{ toCurrency(data.value['Amount Partial PO (RM)']) }}
                </div>
              </template>
              <template v-slot:StatusPO="data">
                {{ data.value['Status PO'] }}
              </template>
              <template v-slot:StatusCancelPOPartial="data">
                {{ data.value['Status Cancel PO Partial'] }}
              </template>
...
<style scoped>
.cancel-partial-table-wrapper :deep(.table-header) {
  display: none;
}

.cancel-partial-table-wrapper :deep(.rs-table thead th) {
