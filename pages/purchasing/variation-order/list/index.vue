...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "List of Variation Order (VO)", path: "/purchasing/variation-order/list" },
  ],
});
...
const filteredVOList = ref([...voList.value]);

const applyFilters = () => {
  let filtered = [...voList.value];

  if (searchKeyword.value && searchKeyword.value.trim() !== "") {
    const keyword = searchKeyword.value.toLowerCase().trim();
    filtered = filtered.filter((item) => {
      const voNo = (item["VO No"] || "").toString().toLowerCase();
  ...
  filteredVOList.value = [];
  nextTick(() => {
    filteredVOList.value = [...filtered];
  });
};

const totalRecords = computed(() => filteredVOList.value.length);
...
const fetchVOs = async () => {
  try {
    ...
    const { data } = await useFetch("/api/purchasing/variation-order/list", {
      method: "GET",
      query: queryParams,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      voList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        "Date of letter": item.date_of_letter || '',
        "VO No": item.vo_no || '',
        "Reference No": item.reference_no || '',
        "Agreement No": item.agreement_no || '',
        "Agreement Ref": item.agreement_ref || '',
        "Amount Agreement (RM)": item.amount_agreement || 0,
        "Amount VO (RM)": item.amount_vo || 0,
        "New Amount Agreement (RM)": item.new_amount_agreement || 0,
        Action: "",
        agg_id: item.agg_id,
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch variation orders",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching variation orders",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDelete = async (item) => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: `Do you want to delete "${item["VO No"]}"?`,
    icon: "warning",
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">List of Variation Order (VO)</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="vo-list-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`vo-list-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredVOList"
              :field="['no', 'Date of letter', 'VO No', 'Reference No', 'Agreement No', 'Agreement Ref', 'Amount Agreement (RM)', 'Amount VO (RM)', 'New Amount Agreement (RM)', 'Action']"
              :options="{
                variant: 'primary',
                striped: false,
                bordered: false,
            ...
            >
              ...
              <template v-slot:Dateofletter="data">
                {{ data.value['Date of letter'] }}
              </template>
              <template v-slot:VONo="data">
                {{ data.value['VO No'] }}
              </template>
              ...
              <template v-slot:AmountVORM="data">
                <div class="text-right">
                  {{ toCurrency(data.value['Amount VO (RM)']) }}
                </div>
              </template>
              <template v-slot:NewAmountAgreementRM="data">
                <div class="text-right">
                  {{ toCurrency(data.value['New Amount Agreement (RM)']) }}
                </div>
              </template>
...
<style scoped>
.vo-list-table-wrapper :deep(.table-header) {
  display: none;
}

.vo-list-table-wrapper :deep(.rs-table thead th) {
