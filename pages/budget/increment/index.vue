...
const filteredIncrementList = ref([...incrementList.value]);
...
const applyFilters = () => {
  let filtered = [...incrementList.value];
  ...
  filteredIncrementList.value = [];
  nextTick(() => {
    filteredIncrementList.value = [...filtered];
  });
};

// Total records count
const totalRecords = computed(() => filteredIncrementList.value.length);
...
// Fetch budget increment records from API
const fetchIncrements = async () => {
  try {
    ...
    const { data } = await useFetch("/api/budget/increment", {
      method: "GET",
      query,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      incrementList.value = (data.value.data || []).map((item, index) => ({
        no: index + 1,
        Year: item.bmm_year,
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch budget increment records",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching budget increment records",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
// Handle add new increment
const handleAdd = () => {
  // For Add mode, only store the mode, no ID needed
  if (typeof window !== 'undefined') {
    sessionStorage.removeItem('increment_form_id'); // Clear any existing ID
    sessionStorage.setItem('increment_form_mode', 'add');
  }
  navigateTo("/budget/increment-v2-form");
};

// Handle view increment
const handleView = (item) => {
  // Store ID in sessionStorage instead of URL
  if (typeof window !== 'undefined') {
    sessionStorage.setItem('increment_form_id', item.bmm_budget_movement_id.toString());
    sessionStorage.setItem('increment_form_mode', 'view');
  }
  navigateTo("/budget/increment-v2-form");
};

// Handle edit increment
...
const handleDownloadPDF = () => {
  ...
  // const query = { ...filters };
  // window.open(`/api/budget/increment/download?format=pdf&${new URLSearchParams(query).toString()}`, '_blank');
};
...
const handleDownloadCSV = () => {
  ...
  // const query = { ...filters };
  // window.open(`/api/budget/increment/download?format=csv&${new URLSearchParams(query).toString()}`, '_blank');
};
...
<template>
  <div class="space-y-6">
    ...
    <!-- Budget Increment Section -->
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Budget Increment</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="budget-increment-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`budget-increment-table-${searchKeyword || 'all'}-${pageSize}`"
              :data="filteredIncrementList"
...
.budget-increment-table-wrapper :deep(.table-header) {
  display: none;
}
</style>
