...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Warrant Increment", path: "/budget/report/warrant/warrant-increment" },
  ],
});
...
watch(filter, () => {
  fetchWarrantIncrement();
}, { deep: true });

const fetchWarrantIncrement = async () => {
  loading.value = true;
  try {
    const query = {};
    ...
    const { data } = await useFetch("/api/budget/report/warrant/warrant-increment", {
      method: "GET",
      query,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch Warrant Increment records",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching Warrant Increment records",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Increment Listing</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="warrant-increment-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`warrant-increment-table-${searchKeyword || 'all'}-${pageSize}`"
...
.warrant-increment-table-wrapper :deep(.table-header) {
  display: none;
}
</style>
