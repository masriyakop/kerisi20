...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Warrant Virement", path: "/budget/report/warrant/warrant-virement" },
  ],
});
...
watch(filter, () => {
  fetchWarrantVirement();
}, { deep: true });

const fetchWarrantVirement = async () => {
  loading.value = true;
  try {
    const query = {};
    ...
    const { data } = await useFetch("/api/budget/report/warrant/warrant-virement", {
      method: "GET",
      query,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      ...
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch Warrant Virement records",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching Warrant Virement records",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Virement Listing</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="warrant-virement-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`warrant-virement-table-${searchKeyword || 'all'}-${pageSize}`"
...
.warrant-virement-table-wrapper :deep(.table-header) {
  display: none;
}
</style>
