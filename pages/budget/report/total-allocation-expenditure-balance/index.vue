<script setup>
definePageMeta({
  title: "Total Allocation, Expenditure and Balance of Allocation",
  middleware: ["auth"],
  requiresAuth: true,
  breadcrumb: [
    ...
    { name: "Total Allocation, Expenditure and Balance of Allocation", path: "/budget/report/total-allocation-expenditure-balance" },
  ],
});
    ...
    <rs-card>
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <div class="total-allocation-expenditure-balance-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`total-allocation-expenditure-balance-table-${searchKeyword || 'all'}-${pageSize}`"
...
.total-allocation-expenditure-balance-table-wrapper :deep(.table-header) {
  display: none;
}
</style>
