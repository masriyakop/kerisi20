<script setup>
definePageMeta({
  title: "Decrement V2 Form",
  middleware: ["auth"],
  requiresAuth: true,
  breadcrumb: [
    { name: "Dashboard", path: "/dashboard" },
    { name: "Budget", path: "/budget" },
    { name: "Decrement V2 Form", path: "/budget/decrement-v2-form" },
  ],
});
...
const bmm_budget_movement_id = computed(() => {
  if (typeof window !== 'undefined') {
    const storedId = sessionStorage.getItem('decrement_form_id');
...
const mode = computed(() => {
  if (typeof window !== 'undefined') {
    return sessionStorage.getItem('decrement_form_mode') || 'add';
  }
  return 'add';
});

// Form data
const decrementForm = ref({
...
const fetchMasterData = async () => {
  ...
  try {
    loading.value = true;
    const { data } = await useFetch(`/api/budget/decrement-v2-form/master`, {
      method: "GET",
...
const fetchDetailList = async () => {
  ...
  try {
    loading.value = true;
    const { data } = await useFetch(`/api/budget/decrement-v2-form/detail`, {
      method: "GET",
      query: { bmm_budget_movement_id: bmm_budget_movement_id.value },
      initialCache: false,
    });
...
const fetchProcessFlow = async () => {
  ...
  try {
    ...
    const { data } = await useFetch(`/api/budget/decrement-v2-form/process-flow`, {
      method: "GET",
      query: { bmm_budget_movement_id: bmm_budget_movement_id.value },
      initialCache: false,
    });
...
const fetchBudgetCodes = async () => {
  ...
  try {
    const { data } = await useFetch(`/api/budget/decrement-v2-form/lookups/budget-codes`, {
      method: "GET",
      query: {
...
watch(() => detailForm.value.sbg_budget_code, async (newVal) => {
  if (newVal) {
    ...
    if (selected) {
      ...
      try {
        const { data } = await useFetch(`/api/budget/decrement-v2-form/detail/get-budget`, {
          method: "GET",
...
const handleEditDetail = async (item) => {
  ...
  try {
    loading.value = true;
    const { data } = await useFetch(`/api/budget/decrement-v2-form/detail/get`, {
      method: "GET",
...
const handleSaveApplication = async () => {
  if (!decrementForm.value.bmm_endorse_doc || !decrementForm.value.bmm_description) {
    $swal.fire({
      title: "Validation Error",
      text: "Please fill in all required fields",
      icon: "warning",
  ...
  if (detailList.value.length === 0) {
    $swal.fire({
      title: "Validation Error",
      text: "Please add at least one detail decrement",
      icon: "warning",
    });
    return;
  }
  ...
  try {
    ...
    const payload = {
      bmm_budget_movement_id: bmm_budget_movement_id.value,
      bmm_year: decrementForm.value.bmm_year,
      bmm_endorse_doc: decrementForm.value.bmm_endorse_doc,
      bmm_description: decrementForm.value.bmm_description,
      bmm_total_amt: detailList.value.reduce((sum, item) => sum + (item.bmd_mvt_amt || 0), 0),
      submitMode: "Save",
    };

    const { data } = await useFetch(`/api/budget/decrement-v2-form/submit`, {
      method: "POST",
      body: payload,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      ...
      if (!bmm_budget_movement_id.value && detailList.value.length > 0) {
        for (const detail of detailList.value) {
          try {
            await useFetch(`/api/budget/decrement-v2-form/detail`, {
              method: "POST",
              body: {
      ...
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('decrement_form_id', newMovementId.toString());
...
// Format decrement amount input
const formatDecrementAmount = (event) => {
...
<template>
  <div class="space-y-6">
    ...
    <!-- Decrement Information Form -->
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Decrement Information</div>
      </template>
      <template #body>
        <div class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <FormKit
              v-model="decrementForm.bmm_year"
              type="select"
              label="Year"
              :options="yearOptions"
            ...
            />
            <FormKit
              v-model="decrementForm.bmm_budget_movement_no"
              type="text"
              label="Reference"
              :disabled="true"
              outer-class="mb-0"
            ...
            />
            <FormKit
              v-model="decrementForm.bmm_endorse_doc"
              type="text"
            ...
            <div class="md:col-span-2 lg:col-span-3">
              <FormKit
                v-model="decrementForm.bmm_description"
                type="textarea"
                label="Remark/Reason"
            ...
            <FormKit
              v-model="decrementForm.bmm_status"
              type="text"
              label="Status"
              :disabled="true"
              outer-class="mb-0"
    ...
    <!-- Detail Decrement Datatable -->
    <rs-card>
      <template #header>
        <div class="flex items-center justify-between">
          <div class="text-lg font-semibold">Detail Decrement</div>
          <rs-button variant="primary" @click="handleAddDetail" class="h-[30px]">
      ...
      <template #body>
        ...
        <rs-table
          v-else
          :data="detailList"
          :field="['No', 'Budget Index No', 'Quarter', 'Fund', 'Activity', 'PTJ', 'Cost Center', 'Budget Code', 'Balance Amount', 'Decrement Amount', 'Action']"
          :options="{
            variant: 'primary',
            striped: false,
            bordered: false,
        ...
        >
          ...
          <template v-slot:DecrementAmount="data">
    ...
    <!-- Workflow Form -->
    <rs-card v-if="bmm_budget_movement_id && decrementForm.bmm_status !== 'DRAFT'">
      <template #body>
        <div class="space-y-4">
    ...
    <rs-modal
      ...
      :title="isEditDetailMode ? 'Edit Detail Decrement' : 'Add Detail Decrement'"
      size="lg"
      :overlay-close="false"
      position="center"
    >
      <template #body>
        <FormKit type="form" :actions="false">
          <div class="space-y-4">
            ...
            <div class="flex items-center gap-4">
              <label class="w-32 text-sm font-medium">Decrement Amount:</label>
              <div class="flex-1 flex items-center">
                <span class="px-3 py-1 bg-gray-100 dark:bg-gray-700 border border-r-0 rounded-l text-sm h-[30px] flex items-center">MYR</span>
                <FormKit
