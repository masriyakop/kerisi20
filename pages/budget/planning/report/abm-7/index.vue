...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "ABM 7", path: "/budget/planning/report/abm-7" },
  ],
});
...
const columns = ref([
  ...
  { key: "oun", label: "PTJ Name", sortable: false },
  { key: "bpm_type", label: "Type", sortable: false },
  { key: "bpm_total_amt", label: "Amount (RM)", sortable: false },
...
const fetchABM7 = async () => {
  ...
  try {
    ...
    if (topFilter.value.type) query.tf_dasar = topFilter.value.type;
    ...
    const { data } = await useFetch("/api/budget/planning/report/abm-7", {
      method: "GET",
      query,
      initialCache: false,
    });
    ...
    if (data.value?.statusCode === 200) {
      abmList.value = (data.value.data || []).map((item, index) => ({
        ...
        bpm_total_amt: item.bpm_total_amt,
      }));
      applyFilters();
    } else {
      $swal.fire({
        title: "Error",
        text: data.value?.message || "Failed to fetch ABM 7 records",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while fetching ABM 7 records",
      icon: "error",
    });
  } finally {
    loading.value = false;
...
const handleDownloadABM7 = (ounCode) => {
  $swal.fire({
    title: "Info",
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <div class="abm-7-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`abm-7-table-${searchKeyword || 'all'}-${pageSize}`"
            ...
            >
              ...
              <template v-slot:bpmtotalamt="data">{{ toCurrency(data.value.bpm_total_amt) }}</template>
              <template v-slot:action="data">
                <div class="flex items-center gap-2 justify-end">
                  <button
                    @click="handleDownloadABM7(data.value.oun_code)"
                    class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"
                    title="Download ABM 7"
                  >
...
.abm-7-table-wrapper :deep(.table-header) {
  display: none;
}
</style>
