...
definePageMeta({
  ...
  breadcrumb: [
    ...
    { name: "Budget Planning Allocation 2 List", path: "/budget/planning/budget-planning-allocation-2-list" },
  ],
});
...
const fetchPlannings = async () => {
  try {
    loading.value = true;
    const { data } = await useFetch("/api/budget/planning/budget-planning-allocation-2-list", {
      method: "GET",
      initialCache: false,
    });
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="text-lg font-semibold">Budget Planning Allocation 2 List</div>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="budget-planning-allocation-2-table-wrapper">
            ...
            <rs-table
              v-else
              :key="`budget-planning-allocation-2-table-${searchKeyword || 'all'}-${pageSize}`"
...
.budget-planning-allocation-2-table-wrapper :deep(.table-header) {
  display: none;
}
</style>
