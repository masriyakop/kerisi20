...
const fetchData = async () => {
  try {
    ...
    const { data: data0 } = await useFetch("/api/message-management/setup/message-type/message-type", {
      method: "GET",
      query,
      initialCache: false,
    });
...
const handleDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      loading.value = true;
      const apiPath = "/api/message-management/setup/message-type/message-type";
