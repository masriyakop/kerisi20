...
definePageMeta({
  ...
  },
  {
    ...
    "path": "/messagemanagement/setup/messagecategory/messagecategory"
  }
],
});
...
const pageNameForLog = "Message Category";
const moduleNameForLog = "Messagecategory";
const pageBreadcrumbTextForLog = "Messagemanagement > Setup > Messagecategory > Message Category";
...
const showMessagecategoryModal = ref(false);
...
const fetchData = async () => {
  try {
    ...
    const { data: data0 } = await useFetch("/api/message-management/setup/messagecategory", {
      method: "GET",
      query,
      initialCache: false,
    });
    ...
    if (data0.value?.statusCode === 200) {
      messagecategoryList.value = (data0.value.data || []).map((item, idx) => {
        const mappedItem = {
          no: idx + 1,
          Action: "",
        };
...
const filteredMessagecategoryList = ref([...messagecategoryList.value]);

// Function to apply filters
const applyFilters = () => {
  let filtered = [...messagecategoryList.value];
  ...
  filteredMessagecategoryList.value = [];
  nextTick(() => {
    filteredMessagecategoryList.value = [...filtered];
  });
};

// Total records count
const totalRecords = computed(() => filteredMessagecategoryList.value.length);
...
const {
  ...
  pageName: "Message Category",
  apiDataPath: "/api/message-management/setup/messagecategory",
  defaultExportColumns: ["Code", "Description", "Status"],
  getFilteredList: () => filteredMessagecategoryList.value,
  datatableRef,
  searchKeyword,
  smartFilter,
  topFilter,
...
const handleView = (item) => {
  ...
  messagecategoryForm.value = { ...item };
  
  showMessagecategoryModal.value = true;
};
...
const handleEdit = (item) => {
  ...
  messagecategoryForm.value = {};
  formFields.forEach((fieldName) => {
    if (item[fieldName] !== undefined) {
      messagecategoryForm.value[fieldName] = item[fieldName];
    }
  });
  showMessagecategoryModal.value = true;
};
...
// Save function
const handleSaveMessagecategory = async () => {
  try {
    loading.value = true;
    const apiPath = "${fallbackApiPath}";
    let response;
    ...
    if (isEditMode.value && editingId.value) {
      ...
      response = await useFetch(`${apiPath}/${editingId.value}`, {
        method: "PUT",
        body: messagecategoryForm.value,
        initialCache: false,
      });
    } else {
      // Add new record
    ...
    if (response.data.value?.statusCode === 200 || response.data.value?.statusCode === 201) {
      ...
      showMessagecategoryModal.value = false;
      messagecategoryForm.value = {};
    } else {
      $swal.fire({
        title: "Error",
...
// Cancel form
const handleCancelMessagecategory = () => {
  showMessagecategoryModal.value = false;
  isViewMode.value = false;
  messagecategoryForm.value = {};
};
...
<template>
  <div class="space-y-6">
    ...
    <rs-card>
      <template #header>
        <div class="flex justify-between items-center">
          <div class="text-lg font-semibold">List of Message Category</div>
          <rs-dropdown
        ...
        <div class="space-y-4">
          ...
          <div class="flex justify-between items-center gap-4 mb-4">
            ...
            <div class="flex items-center gap-2">
              <label class="text-sm font-medium text-gray-700 dark:text-gray-300" for="messagecategory_pageSize">Display:</label>
              <FormKit
                id="messagecategory_pageSize"
                type="select"
                v-model="pageSize"
                :options="[
                  { label: '5', value: 5 },
            ...
            <!-- Search on Right -->
            <div class="flex items-center gap-2">
              <label class="text-sm font-medium text-gray-700 dark:text-gray-300" for="messagecategory_searchKeyword">Search:</label>
              <div class="flex gap-2">
                <FormKit
                  id="messagecategory_searchKeyword"
          ...
          <div class="messagecategory-table-wrapper">
            ...
            <rs-table
              ...
              :key="`messagecategory-table`"
              :data="filteredMessagecategoryList"
    ...
    <!-- Add/Edit Modal -->
    <rs-modal
      v-model="showMessagecategoryModal"
      title="Add/Edit Message Category"
      size="lg"
      dialog-class="messagecategory-modal-custom"
      :overlay-close="true"
      :hide-footer="false"
      position="center"
    >
      <template #header>
        <div class="flex items-center justify-between w-full bg-primary text-white px-3 py-2 rounded-t-lg messagecategory-modal-header">
          <h4 class="text-base font-semibold text-white">
            {{ isViewMode ? 'View Add/Edit Message Category' : (isEditMode ? 'Edit Add/Edit Message Category' : 'Add Add/Edit Message Category') }}
          </h4>
          <Icon
            @click="handleCancelMessagecategory"
            class="hover:text-gray-200 cursor-pointer text-white"
            name="ic:round-close"
            size="18"
          />
      ...
      <template #body>
        <FormKit type="form" :actions="false" @submit="handleSaveMessagecategory">
          <div class="space-y-2 py-2">
            <div class="flex items-center gap-2">
              <label class="w-32 text-xs font-medium" for="messagecategoryForm_Code">Code:</label>
              <div class="flex-1">
                <FormKit
                  id="messagecategoryForm_Code"
                  v-model="messagecategoryForm.Code"
                  type="text"
            ...
            <div class="flex items-center gap-2">
              <label class="w-32 text-xs font-medium" for="messagecategoryForm_Description">Description:</label>
              <div class="flex-1">
                <FormKit
                  id="messagecategoryForm_Description"
                  v-model="messagecategoryForm.Description"
                  type="text"
            ...
            <div class="flex items-center gap-2">
              <label class="w-32 text-xs font-medium" for="messagecategoryForm_Status">Status:</label>
              <div class="flex-1">
                <FormKit
                  id="messagecategoryForm_Status"
                  v-model="messagecategoryForm.Status"
                  type="select"
                  :options="StatusOptions"
                  :disabled="isViewMode"
                  placeholder="Select Status"
      ...
      <template #footer>
        <div class="flex justify-end gap-2 py-2">
          <rs-button variant="danger" size="sm" @click="handleCancelMessagecategory">
            {{ isViewMode ? 'Close' : 'Cancel' }}
          </rs-button>
          <rs-button v-if="!isViewMode" variant="primary" size="sm" @click="handleSaveMessagecategory">
            Save
          </rs-button>
        </div>
      </template>
...
.messagecategory-table-wrapper :deep(.table-header) {
  display: none;
}

/* Left-align Action column header and cells */
.messagecategory-table-wrapper :deep(.rs-table thead th:last-child),
.messagecategory-table-wrapper :deep(.rs-table tbody td:last-child) {
  text-align: left !important;
}
</style>

<style>
/* Custom width for List of Message Category modal - 75% of lg size (800px * 0.75 = 600px) */
.messagecategory-modal-custom {
  width: 600px !important;
}

/* Hide default close icon when custom header is used */
.messagecategory-modal-custom .modal-header > :last-child:not(.messagecategory-modal-header) {
  display: none !important;
}

/* Ensure custom header matches modal content width exactly */
.messagecategory-modal-custom .modal-header {
  padding: 0 !important;
  position: relative;
  overflow: hidden;
}

.messagecategory-modal-custom .messagecategory-modal-header {
  width: 100% !important;
...
/* Ensure Smart Filter modal header matches List of Message Category modal styling */
