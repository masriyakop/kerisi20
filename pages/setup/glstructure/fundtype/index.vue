...
const pageNameForLog = "Fund Type";
const moduleNameForLog = "Fundtype";
const pageBreadcrumbTextForLog = "Setup > Glstructure > Fundtype > Fund Type";
...
const fetchData = async () => {
  try {
    ...
    const { data: data0 } = await useFetch("/api/setup/glstructure/fund-type", {
      method: "GET",
      query,
      initialCache: false,
    });
...
const handleGenerateApiProceed = async () => {
  try {
    ...
    const templateDetails = tableState
      ...
      : {};
    const apiDataPath = "/api/setup/glstructure/fund-type";
...
const handleDownloadPDF = async () => {
  try {
    ...
    const exportColumns = exportConfig ? exportConfig.columns : ["Fund Type","Description","Type Basis","Remark","Status","Entry Date"];
    let dataToExport = exportConfig ? [...exportConfig.data] : [...filteredFundtypeList.value];
...
const handleDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      loading.value = true;
      const apiPath = "/api/setup/glstructure/fund-type";
