...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Base SQL query (contains variables: Fund, PTJ, CostCentre)
    let rawQuery = `SELECT DISTINCT
    at.at_activity_code AS value,
    UPPER(
        CONCAT_WS(
            ' - ',
            at.at_activity_code,
...
FROM
    ...
    activity_type at
WHERE ouc.at_activity_code = at.at_activity_code
    and (
        ouc.fty_fund_type = {Fund}
    )
    and (
        ouc.oun_code = {PTJ}
    )
    and (
        ouc.ccr_costcentre = {CostCentre}
    )
ORDER BY
    at.at_activity_code;`;
    ...
    // Replace ${CostCentre} or {CostCentre} with query.CostCentre value
    if (query.CostCentre !== undefined && query.CostCentre !== null && query.CostCentre !== '') {
      // Check if variable is already quoted in SQL, if not add quotes for string values
      const CostCentreValue = query.CostCentre;
      // Replace ${var} patterns - quoted first (keep the quotes)
      rawQuery = rawQuery.replace(/'\$\{CostCentre\}'/g, "'" + CostCentreValue + "'");
      rawQuery = rawQuery.replace(/"\$\{CostCentre\}"/g, '"' + CostCentreValue + '"');
      rawQuery = rawQuery.replace(/\$\{CostCentre\}/g, "'" + CostCentreValue + "'");
      // Replace {var} patterns (without $) - quoted first (keep the quotes)
      rawQuery = rawQuery.replace(/'\{CostCentre\}'/g, "'" + CostCentreValue + "'");
      ...
      // If variable not provided, first try to remove simple AND/OR conditions
      rawQuery = rawQuery.replace(/AND\s+\w+\s*=\s*'\$\{CostCentre\}'/gi, "");
      rawQuery = rawQuery.replace(/AND\s+\w+\s*=\s*"\$\{CostCentre\}"/gi, "");
      rawQuery = rawQuery.replace(/AND\s+\w+\s*=\s*\$\{CostCentre\}/gi, "");
      rawQuery = rawQuery.replace(/AND\s+\w+\s*=\s*'\{CostCentre\}'/gi, "");
      rawQuery = rawQuery.replace(/AND\s+\w+\s*=\s*"\{CostCentre\}"/gi, "");
      rawQuery = rawQuery.replace(/AND\s+\w+\s*=\s*\{CostCentre\}/gi, "");
      // Then replace any remaining variables with NULL (for complex patterns like: or {var} is null)
      rawQuery = rawQuery.replace(/'\$\{CostCentre\}'/g, "NULL");
      rawQuery = rawQuery.replace(/"\$\{CostCentre\}"/g, "NULL");
      rawQuery = rawQuery.replace(/\$\{CostCentre\}/g, "NULL");
      rawQuery = rawQuery.replace(/'\{CostCentre\}'/g, "NULL");
      rawQuery = rawQuery.replace(/"\{CostCentre\}"/g, "NULL");
