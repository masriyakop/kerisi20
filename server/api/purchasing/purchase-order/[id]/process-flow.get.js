...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Get purchase order to find order number and workflow type
    ...
    // Use workflow type from purchase order, or default to 'PO' as per the SQL query
    const workflowCode = purchaseOrder.pom_wflow_type || 'PO';
    
    // Fetch all processes for the workflow with status '1', ordered by sequence
    // This matches: WHERE wfp_status = '1' AND wfp.wfp_workflow_code = 'PO' ORDER BY wfp_sequence
    ...
    // For each process, fetch application status if it exists for this purchase order
    // This matches: LEFT JOIN wf_application_status was ON (wfp.wfp_process_id = was.was_process_id AND was_application_id = order_no)
